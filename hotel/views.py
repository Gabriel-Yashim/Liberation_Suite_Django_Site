...
from hotel.models import Coupon, CouponUsers, Hotel, Room, Booking, RoomServices, HotelGallery, HotelFeatures, RoomType, Notification, Bookmark, Review, HeroSection, ManagementTeam, ServicesSection, Blogs
...
def room_details(request, slug):
    hotel = Hotel.objects.filter(status="Live")
    # Use get_object_or_404 to retrieve the room_details object or return a 404 response
    room_details = get_object_or_404(RoomType, slug=slug)
    context = {
        "hotel": hotel,
...
def policy(request):
    return render(request, "hotel/policy.html")



def tandc(request):
    return render(request, "hotel/terms_and_conditions.html")
...
def blogs(request):
    ...
    return render(request, "hotel/blogs.html", context)



def blog_details(request, slug):
    hotel = Hotel.objects.filter(status="Live")
    # Use get_object_or_404 to retrieve the Blogs object or return a 404 response
    blog = get_object_or_404(Blogs, slug=slug)
    context = {
        "hotel": hotel,
        "blog": blog,
    }
    return render(request, "hotel/blog_details.html", context)
